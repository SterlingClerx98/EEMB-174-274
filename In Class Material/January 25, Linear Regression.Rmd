...
$$
Transformations among parameters. These use the R `<-` assignmnet, not `=` : 
$$
...
# Additional Exercise
Compare the sampled value of mu for the un-centered and centered models. To do this, you can use the samples of `mu` that we already generated for each model. You can then either put them into a single plot by adding `ggplot` layers using different `data`, or by making a combined dataframe with a column indicating which model was used to generate the estimate.  
...
samples_height_weight_model <-bind_cols(samples_height_weight_model,slice(weights,1:(nrow(samples_height_weight_model)))) %>%
...
```{r}
samples_summarized <- samples_height_weight_model %>%
  group_by(weight)%>%
  summarise(mean_mu=mean(mu),
            lower_mu=quantile(mu,0.1),
            upper_mu=quantile(mu,0.9),
