---
title: "Feb 10, Assessing Model Performance"
...
You all are familar with Hardy-Weinberg principle that single-locus genotypes should show statistical indendendence. This means that if the allele frequency of `A` is $p$ and `a` is $q=1-p$, then the frequency of genotype `AA` should be $p^2$, genotype `Aa` at $2 p q$, and genotype `aa` at $q^2$. 
...
1)  out-of-sample cross entropy is a good measure of distance between model and reality  
...
In modern packages, WAIC and LOOIC can both be efficiently calculated. Both do well at approximating the out-of-sample fit of a model and typically give similar results. 

## Examples of lppd and model complexity

Code in the Rethinking package simulates data and then fits `quap` models with predictors that are not related to the data-generating process.
...
We'll ad another model, one that includes fungus but now leaves out the treatment.
```{r}
m6.7.2 <- quap(
    alist(
        h1 ~ dnorm( mu , sigma ),
...
ggplot(out.long, aes(x=par.delta,y=delta ))+
  geom_violin()+
  geom_point(data=tibble(par.delta=c(1,2,3),delta=c(2,4,8)),
            inherit.aes = FALSE,
            aes(x=par.delta,y=delta),color="red")+
  labs(x="group being compared",
       y="increase in WAIC")
