...
For the final model, we need to specify the standard deviation itself or the model will fail. This is because it will try smaller and smaller values of sigma and run into numerical problems. Here's the trick to get it to run:
```{r  }
## R code 7.9
m7.6 <- quap(
    alist(
...
### overfitting fish
We can see a similar effect in models that are not regression type models. Here we will take the clownfish data from the midterm and show how dividing it up into arbitrary groups of fish always gets better fits to the data. 
```{r}
