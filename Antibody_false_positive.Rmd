...
ggplot(post,aes(x=gamma, y = prob)) +
  geom_line() +
  scale_x_continuous(lim=c(0.965,1))
...
## How about a model where we just mix the parameters ahead of time? This must be a method?




```{r write_mixed_binomial_antibody, eval=FALSE}
sink("model_mixed_binomial_antibody.stan")
cat("
...
pos_pos ~ binomial(n_sample , mu+(1-mu)*(1-gamma));
...
First run the fit with just the data on the pre-covid blood samples. This is a direct way to estimate the false postivie rate. Note that the algorithm still fits the disease prevalence, but it is just the prior.
```{r run_stan_mixed, include=FALSE}


stan_data <-   list( n_pre = 401 , pos_pre = 2 , n_sample=200 , pos_pos = 1 , prior_prev = c(0,1))

# now we are ready to run our first stan program. This will run it with 4 chains for 5000 interations each, which should be plenty
stanfit_bin_mixed <- stan("model_mixed_binomial_antibody.stan", data = stan_data, chains = 8,
                      iter = 15000, seed = 213123)

```

```{r print_1}
print(stanfit_bin_mixed , digits=4)

pairs(stanfit_bin_mixed, pars=c("mu","gamma"))
```
...
stan_data <-   list( n_pre = 401 , pos_pre = 2 , n_sample=22000 , pos_pos = 3500 , prior_prev = c(0,1))
